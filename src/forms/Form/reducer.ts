...
const flattenFilters = [
  {
    test: (_: Fields, value: Field) => {
      const hasValidProperty = Object.prototype.hasOwnProperty.call(
        value,
        'valid'
      )
...
const unflattenRowsFromState = (
  ...
  return {
    unflattenedRows:
      unflattenedRows[path.replace(pathPrefixToRemove, '')] || [],
...
function fieldReducer(state: Fields, action: Action): Fields {
  switch (action.type) {
    ...
    case 'REMOVE_ROW': {
      ...
      const { unflattenedRows, remainingFlattenedState } =
        unflattenRowsFromState(state, path)
